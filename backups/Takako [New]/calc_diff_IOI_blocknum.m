...
%% score information
% % t1 column 4 should be one of them
% From TD_NG
% AA_human_midi =[ 69    73    76    74    73    71    61    64    68    66    64    62    71    74    78    76    74    73    62    66    69    68    66    64    69    71    73    74    73    71    69];
% 
% BC_human_midi =[ 69    71    73    74    73    76    66    68    61    62    64    62    71    73    74    76    74    78    68    69    62    64    66    64    69    71    73    74    73    71    69];
score1_aa = load('Score_set1_AA.txt');
score2_aa = load('Score_set2_AA.txt');
score1_bc = load('Score_set1_BC.txt');
score2_bc = load('Score_set2_BC.txt');
...
for ipair=1:npair
    ...
    for iblock =5:8
        ...
        itask = task_file_all(ipair, iblock);
        iorder = order_file_all(ipair,iblock);
...
%% files that contains mismatching trigger codes that do not fit the order


% ipair=1; 
% iblock=1; %(MH, BC, 229-232, Max leading, thus the order is 2)
% iblock=2; %(MH, BC, 225-228, MH leading, thus the order is 1)
% iblock=3; %(MH, AA, 221-224, Max leading, thus the order is 2)
% iblock=4; %(MH, AA, 217-220, MH leading, thus the order is 1)
% iblock=9; %(CN, BC, 229-232, but Max leading, thus the order is 1)
% iblock=10;%(CN, BC, 225-228, but CN leading, thus, the order is 2)
% iblock=11;%(CN, AA, 221-224, but Max leading, thus, the order is 1)
% iblock=12;%(CN, AA, 217-220, but CN leading, thus, the order is 2)

% ipair=2;
% iblock=1; % (TD, 229-232, but the Curry file name is AA,  Max leading, thus the order is 2)
% iblock=2; % (TD, 225-228, but the Curry file name is AA,  TD leading, thus the order is 1)
% iblock=3; % (TD, 221-224, but the Curry file name is BC,  Max leading, thus the order is 2)
% iblock=4; % (TD, 217-220, but the Curry file name is BC, TD leading, the order is 1
% iblock=9; % (NG, 229-232, but the Curry file name is AA, the trigger contains one 209, Max leading, the
% order is 2)
% iblock=10; % (NG, 225-228, but the Curry file name is AA, NG leading, the
% order is 1)
...
%     69    69
%     
%     >> midi_pair1_block7 = mean(notes_all,2)
% 
% midi_pair1_block7 =
% 
%     69
