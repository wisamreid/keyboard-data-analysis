...
% 
for ith_event_time_idx = 2:ntime_stamps % chop off first 2 (If we want to process files in batches, this is not general enough)
    ...
    %                                                                           
    if  ith_event_time_idx < ntime_stamps & time_stamps(ith_event_time_idx) - time_stamps(ith_event_time_idx - 1) < 1 & (time_stamps(ith_event_time_idx + 1) - time_stamps(ith_event_time_idx) > 17 )

        % store event index
        it_good = [it_good;ith_event_time_idx]; 
    ...
    %   2) the current time stamp - the previous time stamp, is less than one second
    %   
    elseif ith_event_time_idx == ntime_stamps & time_stamps(ith_event_time_idx)-time_stamps(ith_event_time_idx-1) < 1  
