...
% draw
Means=[nanmean(tmpAA_h_pb),nanmean(tmpAA_h_postpb),nanmean(tmpAA_m_pb),nanmean(tmpAA_m_postpb);nanmean(tmpBC_h_pb),nanmean(tmpBC_h_postpb),nanmean(tmpBC_m_pb),nanmean(tmpBC_m_postpb)];
SEMs=[nanstd(tmpAA_h_pb)/sqrt(length(tmpAA_h_pb)),nanstd(tmpAA_h_postpb)/sqrt(length(tmpAA_h_postpb)),nanstd(tmpAA_m_pb)/sqrt(length(tmpAA_m_pb)),nanstd(tmpAA_m_postpb)/sqrt(length(tmpAA_m_postpb));
   nanstd(tmpBC_h_pb)/sqrt(length(tmpBC_h_pb)),nanstd(tmpBC_h_postpb)/sqrt(length(tmpBC_h_postpb)),nanstd(tmpBC_m_pb)/sqrt(length(tmpBC_m_pb)),nanstd(tmpBC_m_postpb)/sqrt(length(tmpBC_m_postpb))];
errorbar_groups(Means, SEMs) ;
legend('AA','BC')
xlabel('Human-partner (pb, post-pb) vs. Max-partner (pb, post-pb)');
ylabel('IOI (ms)');
...
% Results without subj #3
% IV1 is Human vs. Max
% IV2 is AA vs. BC
% IV3 is position (pb, and post-pb
%    
% The number of IV1 levels are: 2
% The number of IV2 levels are: 2
% The number of IV3 levels are: 2
...
% SOV                             SS          df           MS             F        P      Conclusion
% ---------------------------------------------------------------------------------------------------
% Between-Subjects            2715.295         7
% 
% Within-Subjects            20222.117        56
% IV1                         3823.258         1       3823.258        11.051   0.0127        S
% Error(IV1)                  2421.665         7        345.952
% 
% IV2                           44.531         1         44.531         0.076   0.7906       NS
% Error(IV2)                  4093.837         7        584.834
% 
% IV3                         3376.992         1       3376.992        13.247   0.0083        S
% Error(IV3)                  1784.458         7        254.923
% 
% IV1xIV2                      340.826         1        340.826         1.314   0.2893       NS
% Error(IV1xIV2)              1815.279         7        259.326
% 
% IV1xIV3                        1.856         1          1.856         0.012   0.9162       NS
% Error(IV1xIV3)              1091.579         7        155.940
% 
% IV2xIV3                      283.463         1        283.463        13.683   0.0077        S
% Error(IV2-IV3)               145.016         7         20.717
% 
% IV1xIV2xIV3                  137.659         1        137.659         1.121   0.3249       NS
% Error(IV1-IV2-IV3)           859.842         7        122.835
% ---------------------------------------------------------------------------------------------------
% Total                      22935.556        63
% ---------------------------------------------------------------------------------------------------
% With a given significance level of: 0.05
